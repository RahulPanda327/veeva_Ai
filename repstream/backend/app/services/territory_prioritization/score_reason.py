...
def _call_gpt4o(hcp: Dict) -> str:
    ...
    try:
        # Built by the shared factory rather than OpenAI(...) directly. This
        # branch selects its provider with OLLAMA_ENABLED / OPENAI_ENABLED /
        # GROQ_ENABLED and has no LLM_API_KEY or LLM_BASE_URL setting at all, so
        # constructing the client here would raise AttributeError before it ever
        # reached the network. make_llm_client() reads whichever provider is
        # enabled and returns an OpenAI-shaped client, exactly as llm_insight.py
        # does for the sibling ai_generated_insight field.
