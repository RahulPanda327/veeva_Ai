...
def _hcp_detection_dates(db: Session, territory_ids: Optional[List[str]] = None) -> List[datetime]:
    ...
    The bridge is read with raw SQL (only Alert_Id, already proven in
    _affected_hcps_by_alert); the timestamps come from the ActiveAlert ORM so the
    Detection_Datetime column mapping stays in one place. Unparseable values are
    dropped and logged rather than silently counted as datetime.min.
    """
    try:
...
def get_alert_summary(
    ...
    # ai_early_detection_weeks — the detection span across the AFFECTED HCPs: take
    # the detection date of every HCP in the alert→HCP bridge (filter-scoped) and
    # express (max - min) as weeks — (max - min) / 7.
    ...
    # Payer alerts live in payer_access, NOT in insight360_active_alerts_dul, so
    # the ActiveAlert query above cannot see them. The Active Alerts screen DOES
    # show them (as the "Payer" group, carrying change_date in detected_at), so
    # without this the tile silently ignores every payer alert on screen and the
    # span starts at the oldest AI alert instead of the oldest visible one.
    # Same helper and same territory scope get_alerts uses, so both agree.
    detected_dates += [
        d for d in (
            _parse_detected_at(p.detected_at)
            for p in _payer_alerts_from_db(db, territory_ids) if p.detected_at
        )
        if d != datetime.min
    ]
    ...
    if detected_dates:
        ...
        early_detection = round(max(0.0, span_days) / 7, 1)
    else:
        early_detection = 0.0
